                    ...
                    .then((response_patrocinador) => {
                        live_events_service
                            ...
                            .then((response) => {
                                ...
                                setLoading(false);
                                setConferencistas([...new Map(response.data.response.result.filter(item => item.conferencista?.nombre != null).sort((a, b) => a.hora.localeCompare(b.hora)).map(item => [item.conferencista.nombre, item])).values()].filter((item) => item.conferencista.nombre != 'Representante'));
                            })
        ...
        <Stack>
            ...
            <Box sx={{ width: "100%", px: { xs: '5%', md: '8%', lg: '10%' }, py: { xs: '6%', md: '5%' }, backgroundImage: 'url(https://escuela-bancaria.s3.us-east-2.amazonaws.com/a78cbfdc-d866-41e9-8b20-8f4de5418100.jpeg)', backgroundSize: 'cover' }}>
                <Stack spacing={{ xs: 3, md: 6 }}>
                    ...
                    >
                        I CONGRESO REGIONAL PARA LA PREVENCIÓN DE LAVADO DE DINERO U OTROS ACTIVOS Y EL FINANCIAMIENTO DEL TERRORISMO
                    </Typography>
                    <Divider />
                    <Typography
                        variant="h5"
